<template>
  <div>
    <h3 class="text-2xl">Create Or Choose An Album</h3>
    <div class="flex flex-col m-auto w-64">
      <input
        ...
        v-model="albumName"
      />
      <button class="btn-indigo mb-4" @click="createAlbum()">
        Create Album
      </button>
    </div>
    <div class="text-red-500">{{ error }}</div>
    <hr />
    <h5 class="mt-8 text-2xl">List of Albums:</h5>
    <div class="flex flex-wrap w-full m-auto justify-center">
      <div
        v-for="(album, idx) in albums"
        :key="idx"
        class="cursor-pointer mt-4 ml-4 text-indigo-500 bg-indigo-100 w-3/12 h-24 shadow-xl flex items-center justify-center"
